...
def main():
 ...
 'INFLIGHT_SERVICE', 'CLEANLINESS', 'DEPARTURE_DELAY_IN_MINUTES', 'ARRIVAL_DELAY_IN_MINUTES'
       ...
       # 'DAYSINCELASTORDER', 'CASHBACKAMOUNT']
                ...
                if x_column in ( 'GENDER','CUSTOMER_TYPE','TYPE_OF_TRAVEL', 'CLASS','INFLIGHT_WIFI_SERVICE','DEPARTURE_ARRIVAL_TIME_CONVENIENT','EASE_OF_ONLINE_BOOKING',
				'FOOD_AND_DRINK','ONLINE_BOARDING','SEAT_COMFORT','INFLIGHT_ENTERTAINMENT','ON_BOARD_SERVICE', 'LEG_ROOM_SERVICE',
			       'BAGGAGE_HANDLING', 'CHECKIN_SERVICE', 'INFLIGHT_SERVICE', 'CLEANLINESS','GATE_LOCATION'):
                    fig = px.bar(df_g, x=x_column, y="Count",color='SATISFIED_CAT', text=df_g['Percentage'].apply(lambda x: '{0:1.2f}%'.format(x)))
                    st.plotly_chart(fig)

                if x_column in ('AGE','FLIGHT_DISTANCE','DEPARTURE_DELAY_IN_MINUTES','ARRIVAL_DELAY_IN_MINUTES'):
                    fig, ax = plt.subplots()
